...
vector new_vector(int n) {
	int i;
	vector vec = malloc(sizeof(scalar) * (n+MAT_VEC_IDX));

	for (i = MAT_VEC_IDX; i < n+MAT_VEC_IDX; i++) {
		vec[i] = 0.0;
	}
...
void vec_rev(int n, vector vec, vector res) {
	...
	for(i = MAT_VEC_IDX; i < n+MAT_VEC_IDX; i++) {
		res[i] = vec[n+MAT_VEC_IDX-i];
	}
}

void vec_sort(int n, vector vec, vector res) {
	vec_copy(n, vec, res);
	_qsort(res, MAT_VEC_IDX, n+MAT_VEC_IDX-1);
}
...
void mat_add(int n, int m, matrix mat1, matrix mat2, matrix res) {
	...
	for (i = MAT_VEC_IDX; i < n+MAT_VEC_IDX; i++) {
		for (j = MAT_VEC_IDX; j < m+MAT_VEC_IDX; j++) {
...
void mat_prod(int n, int m, matrix mat1, matrix mat2, matrix res) {
	...
	for (i = MAT_VEC_IDX; i < n+MAT_VEC_IDX; i++) {
		for (j = MAT_VEC_IDX; j < m+MAT_VEC_IDX; j++) {
			res[i][j] = 0.0;
			for(k = MAT_VEC_IDX; k < m+MAT_VEC_IDX; k++) {
...
void mat_copyAt(int n, int m, matrix mat, matrix res, int from_x, int from_y, int to_x, int to_y) {
	...
	for (i = from_y; i < n+from_y; i++) {
		for (j = from_x; j < m+from_x; j++) {
			res[i+to_y-MAT_VEC_IDX][j+to_x-MAT_VEC_IDX] = mat[i][j];
		}
	}
}
...
void mat_lu(int n, int m, matrix mat, matrix res) {
    ...
    for (i = MAT_VEC_IDX; i < n+MAT_VEC_IDX-1; i++){
		if (fabs(res[i][i]) < DECOMP_EPS) {
			printf("ピボットが小さすぎてLU分解を続行できません: %f\n", fabs(res[i][i]));
			break;
		}
        for (j = i+1; j < n+MAT_VEC_IDX; j++) {
			res[j][i] /= res[i][i];
			for (k = i+1; k < n+MAT_VEC_IDX; k++) {
...
void mat_chol(int n, int m, matrix mat, matrix resL, vector resD) {
	...
	for (i = MAT_VEC_IDX; i < n+MAT_VEC_IDX; i++) {
		for (j = MAT_VEC_IDX; j < i; j++) {
			resL[j][i] = 0.0;
		}

		resL[i][i] = 1.0;
		sum = 0.0;
		for (k = MAT_VEC_IDX; k < i; k++) {
...
void mat_vec_forward(int n, int m, matrix mat, vector vec, vector res) {
	...
	for (i = n+MAT_VEC_IDX-1; i >= MAT_VEC_IDX; i--) {
		res[i] = vec[i];

		for (j = n+MAT_VEC_IDX-1; j >= i+1; j--) {
			res[i] -= mat[i][j]*res[j];
		}
